        ...
        try {
            form.parse(req, async (error, fields, files) => {
                if (error) {
                    return res
                        .status(500)
                        .json(createResponse("Error parsing the form", false));
                }
                ...
                if (!exerciseName) {
                    return res
                        .status(400)
                        .json(
                            createResponse("Exercise name is required", false)
                        );
                }
        ...
        try {
            form.parse(req, async (error, fields, files) => {
                ...
                for (const routine of allRoutines) {
                    const isSelected = selectedRoutineIds.includes(
                ...
                routine.exercises = routine.exercises?.filter(
                    (exercises) =>
                        (exercises.exercise instanceof mongoose.Types.ObjectId
                            ? exercises.exercise.toString()
                            : exercises.exercise._id?.toString()) !==
                        req.exercise._id.toString()
                );
