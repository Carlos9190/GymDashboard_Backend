...
export class RecordController {
    static createExerciseRecord = async (req: Request, res: Response) => {
        try {
            ...
            res.json(
                createResponse("Exercise record created successfully", true)
            );
        } catch (error) {
    ...
    static getExerciseRecordById = async (req: Request, res: Response) => {
        try {
            res.json(
                createResponse(
                    "Exercise record fetched successfully",
                    true,
                    req.record
                )
            );
    ...
    static updateExerciseRecord = async (req: Request, res: Response) => {
        try {
            ...
            res.json(
                createResponse("Exercise record updated successfully", true)
            );
        } catch (error) {
            res.status(500).json(createResponse("There was an error", false));
        }
    ...
    static deleteExerciseRecord = async (req: Request, res: Response) => {
        try {
            req.exercise.records = req.exercise.records.filter(
                (record) => record._id.toString() !== req.record._id.toString()
            );
            await Promise.allSettled([
                req.record.deleteOne(),
                req.exercise.save(),
            ]);
            res.json(
                createResponse("Exercise record deleted successfully", true)
            );
        } catch (error) {
            res.status(500).json(createResponse("There was an error", false));
        }
