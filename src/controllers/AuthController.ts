...
export class AuthController {
    static createAccount = async (req: Request, res: Response) => {
        try {
            ...
            if (userExists) {
                res.status(409).json(
                    createResponse("Already existing user", false)
                );
                return;
            }
    ...
    static login = async (req: Request, res: Response) => {
        try {
            ...
            if (!isPasswordCorrect) {
                res.status(401).json(
                    createResponse("Incorrect password", false)
                );
                return;
            }
            ...
            res.json(
                createResponse("User authenticated successfully", true, token)
            );
        } catch (error) {
            res.status(500).json(createResponse("There was an error", false));
        }
    ...
    static requestConfirmationCode = async (req: Request, res: Response) => {
        try {
            ...
            await Promise.allSettled([user.save(), token.save()]);
            res.json(
                createResponse("A new token was sent, check your email", true)
            );
        } catch (error) {
            res.status(500).json(createResponse("There was an error", false));
        }
    ...
    static validateToken = async (req: Request, res: Response) => {
        try {
            ...
            res.json(
                createResponse("Valid token, set your new password", true)
            );
        } catch (error) {
            res.status(500).json(createResponse("There was an error", false));
        }
    ...
    static updatePasswordWithToken = async (req: Request, res: Response) => {
        try {
            ...
            await Promise.allSettled([user.save(), tokenExists.deleteOne()]);
            res.json(
                createResponse("The password was changed successfully", true)
            );
        } catch (error) {
            res.status(500).json(createResponse("There was an error", false));
        }
    ...
    static updateCurrentUserPassword = async (req: Request, res: Response) => {
        try {
            ...
            if (!isPasswordCorrect) {
                res.status(401).json(
                    createResponse("Incorrect current password", false)
                );
                return;
            }
