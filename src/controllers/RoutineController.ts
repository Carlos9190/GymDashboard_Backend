...
export class RoutineController {
    ...
    static getAllRoutines = async (req: Request, res: Response) => {
        try {
            ...
            res.json(
                createResponse("Routines fetched successfully", true, routines)
            );
        } catch (error) {
            res.status(500).json(createResponse("There was an error", false));
        }
    ...
    static getCurrentDayRoutine = async (req: Request, res: Response) => {
        try {
            ...
            res.json(
                createResponse(
                    "Routine by current day fetched successfully",
                    true,
                    routines
                )
            );
    ...
    static addExerciseToRoutine = async (req: Request, res: Response) => {
        try {
            ...
            res.json(
                createResponse("Exercise added to routine successfully", true)
            );
        } catch (error) {
            res.status(500).json(createResponse("There was an error", false));
        }
    ...
    static reorderRoutineExercises = async (req: Request, res: Response) => {
        try {
            ...
            const newExercises = orderedExerciseIds.map(
    ...
    static removeExerciseFromRoutine = async (req: Request, res: Response) => {
        try {
            ...
            req.routine.exercises = req.routine.exercises?.filter(
                (exerciseItem) => {
            ...
            res.json(
                createResponse(
                    "Exercise removed from routine successfully",
                    true
                )
            );
        } catch (error) {
